using System;
using System.Collections.Generic;
using System.Linq;
using Ninject.Modules;

namespace Boneject.Filters
{
    public class MultiTypedLoadFilter : ILoadFilter
    {
        private readonly IEnumerable<Type> _moduleTypes;

        public MultiTypedLoadFilter(IEnumerable<Type> moduleTypes)
        {
            Type[]? enumerable = moduleTypes as Type[] ?? moduleTypes.ToArray();

            foreach (Type? type in enumerable)
            {
                if (!typeof(INinjectModule).IsAssignableFrom(type))
                    throw new ArgumentException($"Expected {type.Name} to derive from INinjectModule");
            }

            _moduleTypes = enumerable.ToArray();
        }
        
        public bool ShouldLoad(INinjectModule baseModule)
        {
            return _moduleTypes.Any(type => type == baseModule.GetType());
        }
    }
}
