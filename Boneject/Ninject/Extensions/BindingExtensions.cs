...
public static class BindingExtensions
{
    ...
    /// </summary>
    // ReSharper disable once UnusedMember.Global
    public static IBindingWhenInNamedWithOrOnSyntax<object> BindInterfacesTo(this IBindingRoot self, Type type)
        => self.Bind(type.GetInterfaces()).To(type);
    ...
    public static IBindingWhenInNamedWithOrOnSyntax<T> BindInterfacesAndSelfTo<T>(this IBindingRoot self) 
        => self.Bind(typeof(T).GetInterfaces().Concat(new[] { typeof(T) }).ToArray()).To<T>();
    ...
    /// <returns></returns>
    // ReSharper disable once UnusedMember.Global
    public static IBindingWhenInNamedWithOrOnSyntax<object> BindInterfacesAndSelfTo(this IBindingRoot self, Type type)
        => self.Bind(type.GetInterfaces().Concat(new[] { type }).ToArray()).To(type);
    ...
    public static IBindingWhenInNamedWithOrOnSyntax<T> BindConstant<T>(this IBindingRoot self, T constant)
        => self.Bind<T>().ToConstant(constant);
    ...
    // ReSharper disable once InvalidXmlDocComment
    // ReSharper disable once UnusedMember.Global
    public static IBindingWhenInNamedWithOrOnSyntax<object> BindConstant(this IBindingRoot self, object constant)
        => self.Bind(constant.GetType()).ToConstant(constant);
    ...
    public static void BindConstants(this IBindingRoot self, params object[] constants)
        => Array.ForEach(constants, constant => self.Bind(constant.GetType()).ToConstant(constant).InSingletonScope());
    ...
    public static void BindConstantsInScope(this IBindingRoot self, Func<IContext, object> scope,
        ...
        => Array.ForEach(constants, constant => self.Bind(constant.GetType()).ToConstant(constant).InScope(scope));
}